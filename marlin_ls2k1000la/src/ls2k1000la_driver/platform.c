...
static struct spi_board_info ls1x_spi0_devices[] = {
...
#ifdef CONFIG_SPI_TM7705
    {
        ...
        // ��ǰcpuƵ��=252.00Mhz BUS=126.00Mhz
        // SPI���ķ�Ƶϵ��Ϊ4096
        // ����spi��СƵ��=126Mhz/4096=30.7khz
        // ����ֻ���������Ƶ�ʣ�ʵ��Ƶ��Ϊ��ӽ�����С�����Ƶ�ʵ�BUS��Ƶ���ֵ
        // �������ԣ����Ƶ����ΪΪ1mhzҲ�������ɼ�
...
#ifdef CONFIG_LS1C_PWM_PULSE

static struct resource ls1c_pwm_pulse_resources[] = {
    {
        .start  = LS1X_PWM0_BASE,
        .end    = LS1X_PWM0_BASE + 0x10*4 -1,       // pwm0-3
        .flags  = IORESOURCE_MEM,
    }
};

static struct platform_device ls1c_pwm_pulse = {
    .name           = "ls1c_pwm_pulse",
    .resource       = ls1c_pwm_pulse_resources,
    .num_resources  = ARRAY_SIZE(ls1c_pwm_pulse_resources),
};

#endif // End of CONFIG_LS1C_PWM_PULSE


#ifdef CONFIG_LS1C_3DPRINTER_MOTOR
#include <linux/ls1c_3dprinter_motor.h>

static struct resource ls1c_3dprinter_motor_resources[] = {
    {
        .start  = LS1X_PWM0_BASE,
        .end    = LS1X_PWM0_BASE + 0x10*4 - 1,
        .flags  = IORESOURCE_MEM,
    },{
        .start  = MOTOR_TIMER_IRQ,
        .end    = MOTOR_TIMER_IRQ,
        .flags  = IORESOURCE_IRQ,
    }
};

static struct platform_3dprinter_motor_data ls1c_3dPrinter_motor_data[] = {
    [X_AXIS] = {
...
static struct platform_device ls1c_3dPrinter_motor = 
{
    .name   = "ls1c_3dPrinter_motor",
    .dev    = {
        .platform_data = ls1c_3dPrinter_motor_data,
    },
    .resource       = ls1c_3dprinter_motor_resources,
    .num_resources  = ARRAY_SIZE(ls1c_3dprinter_motor_resources),
};

#endif  // End of CONFIG_LS1C_3DPRINTER_MOTOR


#ifdef CONFIG_LS1C_3DPRINTER_HEATER_FAN
#include <linux/ls1c_3dprinter_heater_fan.h>

static struct platform_3dprinter_heater_fan_data ls1c_3dPrinter_heater_fan_data = {
    .extruder_heater_gpio       = PRINTER_EXTRUDER_HEATER_PIN,
    .extruder_fan_gpio          = PRINTER_EXTRUDER_FAN_PIN,
    .cooling_fan_gpio           = PRINTER_COOLING_FAN_PIN,
};

static struct platform_device ls1c_3dPrinter_heater_fan = {
    .name   = "ls1c_3dPrinter_heater_fan",
    .dev    = {
        .platform_data = &ls1c_3dPrinter_heater_fan_data,
    },
};
...
#endif
#ifdef CONFIG_LS1C_3DPRINTER_MOTOR
    &ls1c_3dPrinter_motor,
#endif
#ifdef CONFIG_LS1C_3DPRINTER_HEATER_FAN
    &ls1c_3dPrinter_heater_fan,
#endif
#ifdef CONFIG_SENSORS_LS1X
	&ls1x_device_hwmon,
#endif
